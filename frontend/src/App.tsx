...
export default function App() {
  ...
  async function go() {
    ...
    try {
      const primaryTicker = (url.startsWith('http') ? url.split('/').pop()! : url).split('?')[0].toUpperCase()
      const peerTickers = peers.split(',').map(t => t.trim().toUpperCase()).filter(Boolean)
      const urls = [primaryTicker, ...peerTickers].map(t => t.startsWith('HTTP') ? t : `https://finance.yahoo.com/quote/${t}`)
      ...
      const pv = Object.entries(bulk.data).filter(([k]) => k !== primaryTicker).map(([ticker, valuation]) => ({ticker, valuation: valuation as ValRes}))
      setPeerVals(pv)
  ...
  return (
    <div className="min-h-screen p-4 bg-gray-100 text-gray-800">
      <div className="max-w-xl mx-auto space-y-3">
        ...
        <input className="w-full p-3 border rounded" placeholder="Yahoo URL or Ticker (e.g., AAPL)" value={url} onChange={e=>setUrl(e.target.value)} />
        <input className="w-full p-3 border rounded" placeholder="Peers (comma-separated, e.g., MSFT, GOOGL)" value={peers} onChange={e=>setPeers(e.target.value)} />
        ...
        {sent && (
          <div className="bg-white rounded shadow p-4">
            ...
            <ul className="list-disc pl-5 mt-1 text-sm">{sent.headlines.map((h,i)=><li key={i}>{h}</li>)}</ul>
          </div>
        )}
      </div>
    </div>
